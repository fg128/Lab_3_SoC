...
module AHBliteTop (
    ...
    output [7:0] segment,   // segment lines to 7-seg display
    output [7:0] digit     // digit enable lines to 7-seg display
...
// Leave any unused interrupt inputs wired to 0 (inactive)
    wire uart_IRQ;
    assign IRQ = {14'b0, uart_IRQ, 1'b0};     // no interrupts in use yet, so all signals 0
...
/* ## Instantiate the GPIO block: AHBgpio.  Connect its bus signals to the bus signals - including
   ...
   output signals to switches, buttons and LEDs as explained in the assignment instructions. */
    AHBgpio GPIO(
       ...
       .gpio_out0   (led_gpio),            // GPIO Out 0, connect to leds
       .gpio_out1   (GPIO_out),            // GPIO Out 1, read-write address 4, nothing connected atm
       .gpio_in0    (sw),                  // GPIO In 0, connect to swicths
       .gpio_in1    ({11'b0, buttons})     // GPIO In 1, connect to buttons, only  5 buttons
       );
...
// ======================= Disaply block ======================================
    AHBdisp DISP (
        ...
        .HWDATA     (HWDATA),
        .HRDATA     (HRDATA_disp),
        .HREADYOUT  (HREADYOUT_disp),
        .digit      (digit),       // active low digit enables
        .segment    (segment)       // active low segment lines
    );
